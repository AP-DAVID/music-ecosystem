      ...
      <div>
        ...
        <div className="flex cursor-pointer items-center align-middle h-12 bg-gray-100 w-96 rounded-r-full rounded-l-full border-2">
          <div
            ...
            className={`flex cursor-pointer h-full w-1/2 justify-center rounded-r-full rounded-l-full items-center align-middle ${
              !change ? "shadow-md bg-gray-900 " : "bg-gray-100"
            }`}
          >
            <h2
          ...
          <div
            ...
            className={`flex cursor-pointer rounded-r-full rounded-l-full h-full w-1/2 justify-center items-center align-middle ${
              change ? "shadow-md bg-gray-900" : "bg-gray-100"
            }`}
          >
            <h2
        ...
        <RefreshIcon
          onClick={refreshNow}
          className={`h-7 ml-3 mt-3 w-7 text-right ${
            spin ? "animate-spin" : "animate-none"
          } text-blue-500`}
        />

        {!change ? (
          <div className="sm:px-5  my-10 sm:mr:10 justify-items-stretch sm:space-x-3 sm:grid md:grid:cols-2 lg:grid-cols-2 xl:grid-cols-3 3xl:flex flex-wrap justify-center">
            {typeof data !== "undefined" && data.length === 0 ? (
              <h1 className="text-gray-500 text-center mt-16">
                follow a user to view their tweets
              </h1>
            ) : (
              data?.map((content) => (
          ...
          <Profiletweet data={data} session={session} />
        )}
      </div>

      <div
        onClick={() => setShowModal(true)}
        className="px-2 py-2 rounded-full  cursor-pointer fixed p-14 bottom-8 right-6 space-y-2 text-align group"
      >
        <div className="invisible group-hover:visible  ">
          <h1 className="text-sm text-gray-500 font-md">tweet</h1>
        </div>
        <div>
          <Icon
