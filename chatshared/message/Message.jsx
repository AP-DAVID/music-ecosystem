        ...
        <div className = {own ? "message own" : styles.message}>
            ...
            <div className={styles.messageTop}>
              <div style={{display:'flex',flexDirection : "row"}}>
               {!own && ( <img className = {styles.messageImg} src={`${message.profile ? message.profile : "/image/nope.jpg"}`} alt=""/>)}
                <p className ={ `messageText ${more ? "line-clamp-6" : "line-camp-none"}`}>{message.text}</p>
                
                {own && ( <img className = {styles.mesage} src={`${message.profile ? message.profile : "/image/nope.jpg"}`} alt=""/>)}
             </div>
