...
function TweetModal({ setShowModal, session, refreshData }) {
  ...
  // The preset for the application
  ...
  // form to get the music tweet data
  ...
  const handleSubmit = async () => {
    if (form.text === "") {
      ...
      if (imagee != "") {
        //   take the music tweet picture, post to cloudinary and save the image link gotten to our api
        try {
          setValid(true);
        ...
        <div className="relative sm:w-2/3 my-auto mx-auto max-w-full">
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            ...
            <div className="flex items-center justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
              ...
              <div>
                <img src="/twitter.svg" className="h-14 w-14" />
              </div>
            </div>
            {/*body*/}

            <div>
              <h1 className="text-center text-red-400 text-base ">{message}</h1>{" "}
            </div>
              ...
              <div className=" items-center ">
                <div>
                  <img
                    ...
                    className="ml-2 mt-2 mb-2 h-16 w-16 object-cover rounded-full cursor-pointer"
                  />
                </div>
                <div>
                  <h1 className="ml-4 text-sm text-blue-500">
                    {session?.user?.username}
                  </h1>
                </div>
              </div>

              {/* includes the textarea for the music tweet text  */}
              <div>
                <TextArea
                  showCount
                  cols="100"
            ...
            {/* contains the section to retrieve the music image from the user*/}
            <div className="flex w-full items-center justify-between p-6 border-t border-solid border-blueGray-200 rounded-b">
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  <div>
                    {" "}
                    <h1 className="text-blue-500 text-sm">Tweet</h1>{" "}
                  </div>
                  <div>
                    <img
                      src="/twitter.svg"
