...
export default function Chatbox({
  ...
  useEffect(async () => {
    ...
    const lastIndex = await findLastIndex(
      messages,
      (v) => v.sender != user._id
    );
    ...
    <div className={`${styles.messenger} bg-white h-full w-full`}>
      <div className={styles.chatBox}>
        <div className={styles.chatBoxWrapper}>
          <div
            className="w-12 h-10 top:0 rounded-full"
            style={{ backgroundColor: "#00838F" }}
          >
            <img
              src={pic ? pic : "/image/noAvatar.png"}
          ...
          <div className={`${styles.chatBoxBottom} mt-6 bottom-0`}>
            ...
            <IconButton aria-label="send" onClick={handleSubmit}>
              <div className="">
                ...
                <PaperAirplaneIcon
                  style={{ backgroundColor: "#00838F" }}
                  className="h-16 sm:h-20 text-center rounded-full px-2 py-1 bg-blue-300 w-15 mr-5 text-white"
                />{" "}
              </div>
            </IconButton>
          </div>
